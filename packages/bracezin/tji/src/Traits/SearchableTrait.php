...
trait SearchableTrait
{
    ...
    protected function makeGroupBy(Builder $query)
    {
        if ($groupBy = $this->getGroupBy()) {
            ...
            // $morphJoins = array_keys(($this->getMorphJoins()));
            // if($morphJoins && count($morphJoins) > 0) {
            //     foreach ($this->getColumns() as $column => $relevance) {
            //         $array = array_map(function ($morphJoins) use ($column, $query) {
            //             if (Str::contains($column, $morphJoins)) {
            //                 $query->groupBy($column);
            //             }
            //         }, $morphJoins);
            //     }
